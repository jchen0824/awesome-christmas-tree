  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Use the WASM folder from JSDelivr matching the version in import map
        ...
        try {
            ...
            console.warn("Failed to initialize with GPU delegate, falling back to CPU:", gpuError);
            try {
                handLandmarkerRef.current = await HandLandmarker.createFromOptions(vision, {
                baseOptions: {
                    modelAssetPath: "https://storage.googleapis.com/mediapipe-models/hand_landmarker/hand_landmarker/float16/1/hand_landmarker.task",
                ...
                console.error("Failed to initialize MediaPipe with CPU delegate:", cpuError);
                throw cpuError;
            }
        }
